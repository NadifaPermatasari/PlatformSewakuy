...
ready(() => {
  ...
  window.loadSellerItems = async function(){
    ...
    try{
      ...
      snap.forEach(docSnap => {
        ...
        // FIX: tambahkan ID Firestore supaya fungsi Edit/Hapus aman
        it.id = docSnap.id;
  ...
  window.loadRentHistory = async function(){
    ...
    try{
      ...
      for(const docSnap of snap.docs){
        ...
        // FIX: tambahkan rentId = docSnap.id
        r.rentId = docSnap.id;
            ...
            <div class="row">
              ...
              <div class="col-md-9">
                ...
                ${r.status === "Sedang Disewa"
                  ? `<button class="btn btn-success btn-sm" onclick="returnItem('${r.rentId}', '${r.itemId}')">Tandai Dikembalikan</button>`
                  : ""
                }
              </div>
            </div>
  ...
  window.editItem = async function(itemId){
    try{
      ...
      await docRef.update({
        nama: newName,
        harga: Number.isFinite(newHarga) ? newHarga : it.harga,
        stok:  Number.isFinite(newStok)  ? newStok  : it.stok
      });
  ...
  window.deleteItem = async function(itemId){
    ...
    try{
      ...
      alert("Barang & transaksi terkait dihapus.");
    }
    catch(err){
      console.error(err);
  ...
  /* =============================
     RETURN ITEM (Dikembalikan)
  ============================== */
  window.returnItem = async function(rentId, itemId){
    ...
    try{
      ...
      alert("Barang dikembalikan & stok diperbarui.");
    }
    catch(err){
      console.error(err);
  ...
  console.log("seller.js loaded & fixed!");
